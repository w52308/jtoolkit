#!/bin/bash
# JVM工具箱入口
# @author: qufengfu@gmail.com

echo ""
echo ">>>>>>>>>>JVM工具箱<<<<<<<<<<"
echo "1.JVM参数检查"
echo "2.JVM参数生成"
echo "0.返回上级菜单"
echo "q.退出"
echo ""
read -p "请输入:" num

is_number(){
  regex='^[0-9]+$'
  if ! [[ $1 =~ $regex ]] ; then
    echo "false"
  else
    echo "true"
  fi
}

if [[ $num == 'q' ]]; then
  echo "Goodbye"
elif [[ $num -eq '0' ]]; then
  cd ..
  source ./jtoolkit.sh
elif [ $num -eq '1' ];then
  if [ ! -f "jvm_option_check.sh" ]; then
    echo "正在下载jvm_option_check.sh......"
    sudo wget  --no-cache http://3.106.11.105:8083/tool/jvm/jvm_option_check.sh >> /dev/null 2>&1
  fi
  sh jvm_option_check.sh

  #在当前进程执行
  source ./sub_menu.sh
elif [ $num -eq '2' ];then
  if [ ! -f "jvm_option_generate.sh" ]; then
    echo "正在下载jvm_option_generate.sh......"
    sudo wget  --no-cache http://3.106.11.105:8083/tool/jvm/jvm_option_generate.sh >> /dev/null 2>&1
  fi
  sh jvm_option_generate.sh

  #在当前进程执行
  source ./sub_menu.sh
fi
